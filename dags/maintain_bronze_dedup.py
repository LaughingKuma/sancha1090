...
@dag(
    ...
    description="Daily OPTIMIZE FINAL on the ReplacingMergeTree bronze tables (opensky_states + adsb_states) so a replay surplus can't accumulate un-merged",
...
def maintain_bronze_dedup():
    ...
    def optimize() -> dict:
        ...
        # daily merge on both RMT tables (each self-skips a not-yet-RMT table, so no churn pre-migration). Raises.
        from include.clickhouse import optimize_states_final

        return {t: optimize_states_final(t) for t in ("opensky_states", "adsb_states")}

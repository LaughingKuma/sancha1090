...
import { enterFocus, exitFocus, dropFocus, isFocused, focusedKey } from "./focus.js";
import { fetchInstances } from "./data.js";
import { pickCandidate } from "./resolve.js";
...
// Feature-internal state: the rail's whole world — the map is reached only through the facade.
export const W = {
  view: "overview",
  range: "30d",
  airline: null,
...
export const TIER_LABEL = { settled: "● STL", estimated: "◐ EST", provisional: "○ PRV", none: "· NONE", unknown: "· UNK" };
const TIER_GLYPH = { settled: "●", estimated: "◐", provisional: "○", none: "·" };
const PRESETS = { "7d": 7, "30d": 30, "90d": 90 };
const VIEWS = { overview, drill, log, flags, trends, estimates, coverage };
...
export function renderView() {
  if (!W.body) return;
  const mod = VIEWS[W.view] || VIEWS.overview;
...
export function syncChrome() {
  if (!W.rail) return; // state can move before the chrome exists (the node harness drives navigate() bare)

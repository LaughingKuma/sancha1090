...
EXPECTED_DAGS = {
    ...
    "ch_serving_parity": {
        ...
        "catchup": False,
        "max_active_runs": 1,
        "task_ids": {"gate", "value_gate", "path_coverage"},
        # gate -> value_gate so the watermark advances only after bronze completeness passes (its oracle is
        # bronze) — else a meaningless pass ages discrepancies out of the recheck window. path_coverage is
        # a parallel leaf: its red must never stop the value gate.
        "downstream_task_ids": {
            "gate": {"value_gate", "path_coverage"},
        },

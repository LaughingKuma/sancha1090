...
def _rotation_pair_store(partial_kw=None, complete_kw=None, *, partial_data=True):
    # Canonical dead-writer pair — incomplete partial (uuid-A, T09) + complete successor (uuid-B, T10);
    # overrides land on EITHER manifest, and the yielded-filename set comes back for the assert.
    partial = dict(process_uuid="uuid-A", process_start_ts="2026-05-29T09:00:00Z",
                   complete=False, row_count=100)
    partial.update(partial_kw or {})
    complete = dict(process_uuid="uuid-B", process_start_ts="2026-05-29T10:00:00Z", row_count=200)
    complete.update(complete_kw or {})
    store = {
        _key("adsb_state", f"{PARTIAL_STEM}.manifest.json"): _manifest_bytes(
            f"{PARTIAL_STEM}.parquet", "adsb_state", **partial),
        _key("adsb_state", f"{COMPLETE_STEM}.parquet"): b"Y",
        _key("adsb_state", f"{COMPLETE_STEM}.manifest.json"): _manifest_bytes(
            f"{COMPLETE_STEM}.parquet", "adsb_state", **complete),
    }
    if partial_data:
        store[_key("adsb_state", f"{PARTIAL_STEM}.parquet")] = b"X"
    return {b.filename for b in ad.list_remote_bundles(FakeFS(store), BUCKET)}
...
def test_superseded_partial_from_dead_process_is_yielded():
    ...
    assert _rotation_pair_store() == {f"{PARTIAL_STEM}.parquet", f"{COMPLETE_STEM}.parquet"}


def test_partial_from_newest_process_not_yielded():
    # Live in-progress case (time order reversed): no complete bundle is newer, so the partial
    # can't be proven dead.
    filenames = _rotation_pair_store(
        partial_kw={"process_start_ts": "2026-05-29T10:00:00Z"},
        complete_kw={"process_start_ts": "2026-05-29T09:00:00Z"})
...
def test_partial_same_uuid_as_complete_not_yielded():
    ...
    assert _rotation_pair_store(complete_kw={"process_uuid": "uuid-A"}) == {f"{COMPLETE_STEM}.parquet"}
...
def test_superseded_partial_missing_data_file_not_yielded():
    # Superseded-by-identity still must clear the ordinary integrity guards: no parquet for the
    # partial — data never landed.
    assert _rotation_pair_store(partial_data=False) == {f"{COMPLETE_STEM}.parquet"}


def test_partial_without_start_ts_not_yielded():
    assert _rotation_pair_store(partial_kw={"process_start_ts": None}) == {f"{COMPLETE_STEM}.parquet"}


def test_all_complete_without_start_ts_partial_not_yielded():
    assert _rotation_pair_store(complete_kw={"process_start_ts": None}) == {f"{COMPLETE_STEM}.parquet"}
...
def test_partial_with_naive_start_ts_not_yielded_no_exception():
    ...
    filenames = _rotation_pair_store(partial_kw={"process_start_ts": "2026-05-29T09:00:00"})
    assert filenames == {f"{COMPLETE_STEM}.parquet"}
...
def test_partial_with_missing_process_uuid_not_yielded():
    ...
    assert _rotation_pair_store(partial_kw={"process_uuid": None}) == {f"{COMPLETE_STEM}.parquet"}
...
def test_partial_with_non_string_start_ts_not_yielded_no_exception():
    ...
    assert _rotation_pair_store(partial_kw={"process_start_ts": 123}) == {f"{COMPLETE_STEM}.parquet"}


def test_complete_with_non_string_start_ts_partial_not_yielded_no_exception():
    assert _rotation_pair_store(complete_kw={"process_start_ts": 123}) == {f"{COMPLETE_STEM}.parquet"}
...
def test_complete_with_missing_process_uuid_does_not_supersede():
    ...
    assert _rotation_pair_store(complete_kw={"process_uuid": None}) == {f"{COMPLETE_STEM}.parquet"}

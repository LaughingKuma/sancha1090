...
live_tail as (
    ...
    having snapshot_hour > (
        select coalesce(max(snapshot_hour), {% if target.type == 'clickhouse' %}toDateTime('1970-01-01 00:00:00', 'UTC'){% else %}timestamp '1970-01-01 00:00:00 UTC'{% endif %}) from live_archive
    )
),
history as (
    select h.*
    ...
    where la.snapshot_hour is null
      ...
      -- CH DateTime maxes at 2106 so the Trino '9999' sentinel overflows; an empty live_tail (count()=0)
      -- means "include all history", else keep history strictly older than live_tail's oldest hour.
      and (
